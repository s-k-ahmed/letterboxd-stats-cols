...
window.onload = () => {
    console.log('window loaded - start of inject script')
    setTimeout(() => {
        getCountryWatches();
        fillCountryPaths();
    }, 1000);
...
function getCountryWatches() {
    // let scripts = document.querySelectorAll('script');
    // let scriptCountryWatch;
    // scripts.forEach(s => {
    //     let t = s.innerText;
    //     if (t.indexOf('allCountriesWatched') != -1) {
    //         scriptCountryWatch = s;
    //     }
    // })
    // let scriptTxt = scriptCountryWatch.innerText;
    // let countryTxt = scriptTxt.match(/(?<=var allCountriesWatched = )\{[^£]*\}(?=;\s*var watchedCountries)/g) + "";
    // countryTxt = countryTxt.replaceAll("'", '"').replaceAll('count:', '"count":').replaceAll('label:', '"label":').replaceAll('url:', '"url":');
    let countryTxt = document.querySelector('#film-world-map').getAttribute('data-all-countries-watched');
    window.allCountriesWatched = JSON.parse(countryTxt);
    console.log(window.allCountriesWatched);
}

const acwCounts = [];

function fillCountryPaths() {
    ...
    [...countryPaths].forEach((path, i) => { 
        ...
        let dataid = path.__data__.properties.iso2cd;
        if (Object.keys(window.allCountriesWatched).indexOf(dataid) == -1) {
            return;
        }
